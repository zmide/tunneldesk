...
async function main() {
  ...
  ok("SSH 连接分组标题支持滚动冻结和阴影分层", appCss.includes(".group-head-row.connection-group-head-row") && appCss.includes("position:sticky") && appCss.includes("background:var(--sidebar)") && appCss.includes("box-shadow:0 1px 0 var(--line)"));
  ...
  ok("窄屏终端工具栏压缩文字且保留全部按钮访问", appCss.includes("container-type:inline-size") && appCss.includes("@container (max-width:700px)") && appCss.includes("justify-content:safe flex-end") && appCss.includes(".terminal-actions > button:not(.terminal-dropdown-button) > span"));
  ...
  ok("桌面工作区标题、标签与内容留白使用紧凑高度", appCss.includes("grid-template-rows:42px minmax(0,1fr)") && appCss.includes(".topbar { height:42px") && appCss.includes(".tabs { height:32px") && appCss.includes(".workspace { padding:12px") && appCss.includes(".content.terminal-content .workspace { padding:6px 12px 12px"));
