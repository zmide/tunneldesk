...
function persistJobs() {
  ...
  fs.writeFileSync(JOBS_FILE, JSON.stringify({ jobs: next }, null, 2), "utf8");
}
...
function writeJsonJobs(items) {
  fs.mkdirSync(DATA_DIR, { recursive: true });
  fs.writeFileSync(JOBS_FILE, JSON.stringify({ jobs: items.slice(0, MAX_HISTORY) }, null, 2), "utf8");
}
...
function getRemoteSize(connection, remotePath) {
  const child = spawnRemote(connection, `wc -c < ${shellQuote(remotePath)} | tr -d ' '`);
...
function runDownloadJob(id, fetchSize) {
  ...
  (async () => {
    try {
      ...
      const command = offset > 0 ? `tail -c +${offset + 1} ${shellQuote(job.remote_path)}` : `cat -- ${shellQuote(job.remote_path)}`;
      const child = spawnRemote(connection, command);
...
function resumeUploadJob(id) {
  ...
  const command = transferred > 0 ? `cat >> ${shellQuote(job.remote_path)}` : `cat > ${shellQuote(job.remote_path)}`;
  const child = spawnRemote(connection, command);
...
function copyJob(connectionId, paths, targetDir) {
  ...
  return startSftpJob(connectionId, "copy", `cp -a -- ${quoted} ${shellQuote(targetDir)}`, `复制 ${paths.length} 项`);
}

function moveJob(connectionId, paths, targetDir) {
  const quoted = (paths || []).map(shellQuote).join(" ");
  if (!quoted) throw new Error("请选择要移动的文件");
  return startSftpJob(connectionId, "move", `mv -- ${quoted} ${shellQuote(targetDir)}`, `移动 ${paths.length} 项`);
}
...
function normalizeCompressionRequest(paths, targetDir = ".", archiveName = "") {
  ...
  const command = `if [ -e ${shellQuote(output)} ]; then echo "目标压缩包已存在" >&2; exit 1; fi; tar -czf ${shellQuote(temporaryOutput)} -C ${shellQuote(parent)} -- ${names.map(shellQuote).join(" ")} && mv -- ${shellQuote(temporaryOutput)} ${shellQuote(output)} || { status=$?; rm -f -- ${shellQuote(temporaryOutput)}; exit $status; }`;
...
function compressJob(connectionId, paths, targetDir = ".", archiveName = "") {
  const request = normalizeCompressionRequest(paths, targetDir, archiveName);
  return { ...startSftpJob(connectionId, "compress", request.command, `压缩 ${request.paths.length} 项为 ${request.name}`), output:request.output };
}

module.exports = { cancelSftpJob, clearFinishedSftpJobs, compressJob, copyJob, deleteSftpJob, extractJob, getSftpJobFile, listSftpJobs, moveJob, normalizeCompressionRequest, pauseSftpJob, resumeSftpJob, startDownloadJob, startUploadJob, __buildCompressCommand: normalizeCompressionRequest };
