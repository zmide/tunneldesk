...
async function main() {
  ...
  ok("启动脚本在依赖清单变化后自动安装", startBat.includes("scripts\\dependency-state.js") && read("start.sh").includes("scripts/dependency-state.js") && dependencyState.includes("package-lock.json") && dependencyState.includes(".tunneldesk-dependencies.sha256"));
  ...
  ok("工作区标签支持即时拖动排序、内容自适应标题和持久化", frontend.includes("beginWorkspaceTabDrag") && frontend.includes("WORKSPACE_TAB_DRAG_THRESHOLD = 5") && frontend.includes("reorderWorkspaceTabElement") && frontend.includes("updateWorkspaceTabAutoScroll") && frontend.includes("saveTabsState") && frontend.includes("向左移动") && frontend.includes("向右移动") && frontend.includes("handleWorkspaceTabsWheel") && frontend.includes("updateWorkspaceTabScrollControls") && frontend.includes('const fullTitle = [tab.title, tab.subtitle]') && frontend.includes('closest(".tabs-shell")?.clientWidth') && indexHtml.includes('id="tabsScrollLeft"') && indexHtml.includes('id="tabsScrollRight"') && appCss.includes("touch-action:pan-y") && appCss.includes(".tab.tab-dragging") && appCss.includes("min-width:92px") && appCss.includes("margin-left:0") && appCss.includes("font-size:12px") && appCss.includes(".tabs::-webkit-scrollbar { display:none; }") && appCss.includes(".tabs-scroll-button[hidden] { display:none; }"));
  ok("窄屏终端工具栏分行、左对齐并保留全部操作", appCss.includes("container-name:terminal-view") && appCss.includes("container-name:terminal-toolbar") && appCss.includes("@container terminal-view (max-width:1080px)") && appCss.includes("@container terminal-toolbar (max-width:1080px)") && appCss.includes("@container terminal-toolbar (max-width:700px)") && appCss.includes(".terminal-title-row { flex:1 1 100%; width:100%; }") && appCss.includes(".terminal-actions { justify-content:flex-start; }") && appCss.includes(".terminal-actions > button:not(.terminal-dropdown-button) > span { display:none; }"));
  ...
  ok("终端编码与字体使用独立下拉菜单并持久化", terminalFrontend.includes("showTerminalEncodingMenu") && terminalFrontend.includes("showTerminalFontMenu") && terminalFrontend.includes("applyTerminalPreferences") && terminalFrontend.includes("focusTerminalSession") && terminalFrontend.includes("terminal-preferences") && terminalFrontend.includes("terminal_font_family") && dbSource.includes("terminal_encoding TEXT NOT NULL DEFAULT 'utf8'") && dbSource.includes("terminal_font_size INTEGER NOT NULL DEFAULT 13"));
  ...
  ok("SFTP 文本编辑支持检测、切换并保持编码", read("src/sftp.ts").includes("decodeRemoteText") && read("src/sftp.ts").includes("encodeRemoteText") && read("public/app-sftp.js").includes("sftpTextEncodingOptions") && read("public/app-sftp.js").includes("persist_default") && dbSource.includes("sftp_text_encoding TEXT NOT NULL DEFAULT 'auto'"));
  ok("SFTP 文件名编码独立切换并持久化", read("src/sftp.ts").includes("decodeRemoteFilenameOutput") && read("src/sftp.ts").includes("remotePathOperand") && read("public/app-sftp.js").includes("showSftpFilenameEncodingMenu") && dbSource.includes("sftp_filename_encoding TEXT NOT NULL DEFAULT 'utf8'"));
  ...
  ok("SFTP 文件夹大小按需递归读取精确字节并兼容 GNU/BSD stat", sftpBackend.includes("buildRemoteDirectorySizeCommand") && sftpBackend.includes("stat -c '%s'") && sftpBackend.includes("stat -f '%z'") && sftpBackend.includes('find "$TD_TARGET" -type f') && sftpBackend.includes("未返回不完整大小") && !sftpBackend.includes("du -") && serverSource.includes('parts[4] === "directory-size"') && sftpFrontend.includes("sftpDirectorySizeButtonHtml") && sftpFrontend.includes("/sftp/directory-size"));
  ok("终端与 SFTP 可以按连接双向跳转", terminalFrontend.includes("openSftp(${c.id})") && read("public/app-sftp.js").includes("openTerminal(${id})"));
  ...
  ok("桌面工作区标题、标签与内容留白使用紧凑高度", appCss.includes("grid-template-rows:42px minmax(0,1fr)") && appCss.includes(".topbar { height:42px") && appCss.includes(".tabs-shell { height:32px") && appCss.includes(".workspace { padding:12px") && appCss.includes(".content.terminal-content .workspace { padding:6px 12px 12px"));
