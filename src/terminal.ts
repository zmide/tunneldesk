...
function handleTerminalUpgrade(req, socket) {
  ...
  try {
    ...
    sendFrame(socket, `连接到 ${connection.ssh_user}@${connection.ssh_host}:${connection.ssh_port}${session.ptyProcess || session.remotePty ? "（PTY）" : ""}\r\n`);

    let pending = Buffer.alloc(0);
    socket.on("data", (chunk) => {
      try {
        pending = parseFrames(Buffer.concat([pending, chunk]), (opcode, payload) => {
          if (opcode === 8) return closeTerminalSession(session);
          if (opcode === 9) return sendFrame(socket, payload, 10);
...
function startRemotePty(connection, socket, cols, rows, log, fallback = null) {
  const session: any = { socket, remotePty: true, ssh2Client: null, ssh2Stream: null, pendingInput: [], logFile: log.fullPath };
...
function startTerminalProcess(connection, socket, cols, rows, title = "") {
  ...
  if (pty) {
    try {
      ...
      const session = { socket, ptyProcess, logFile: log.fullPath };
      ...
      if (process.platform === "darwin") {
        sendFrame(socket, `PTY 启动失败，正在尝试内置 SSH PTY：${error.message}\r\n`);
        return startRemotePty(connection, socket, cols, rows, log, { args, cwd, input: [] });
      }
      sendFrame(socket, `PTY 启动失败，已自动切换普通终端：${error.message}\r\n`);
    }
  } else if (process.platform === "darwin") {
    sendFrame(socket, `PTY 组件不可用，正在尝试内置 SSH PTY：${ptyLoadError || "node-pty 未安装"}\r\n`);
    return startRemotePty(connection, socket, cols, rows, log, { args, cwd, input: [] });
  }

  return startPlainTerminal({ socket, child: null, logFile: log.fullPath }, connection, args, cwd, log);
}
...
function writeTerminalInput(session, payload) {
  ...
  else if (session.ssh2Stream) session.ssh2Stream.write(payload);
  else if (session.remotePty) session.pendingInput.push(Buffer.from(payload));
  else session.child?.stdin.write(payload);
}
