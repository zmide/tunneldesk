...
- 恢复数据库时会按连接列出原私钥名称，可从当前密钥目录或用户 `~/.ssh` 选择私钥，也可上传新私钥；实际文件不要求与原名称相同，可勾选连接、测试连通性并暂存绑定后继续恢复
...
- SFTP 目录浏览兼容 Linux、Termux 和 macOS 的远端命令差异，不依赖 GNU `find` 专有参数
...
- 支持 512 KB 以内、任意扩展名的 UTF-8 普通文件以文本方式打开和编辑，包含行数/大小统计、差异预览、原字节备份、`Ctrl+S` 保存、超限保护和未保存关闭提醒
- 支持设置活动栏；左侧操作区以固定纵向单列显示通用设置、安全设置、通知设置、启动与运行和关于。通用设置包含数据存储、桌面端行为和 SFTP 回收站，安全设置集中管理 Web 认证、密码、Token 和配置加密
- “启动与运行”支持多选 `127.0.0.1`、全部 IPv4 网卡和检测到的网卡 IP，支持手动端口、占用检查、端口建议和重启生效提示；同页展示当前实际本机/LAN 访问地址
- 支持加密迁移包导出和恢复，用于跨机器迁移已启用配置加密的数据
- 支持服务器标签，并可通过搜索按标签过滤
...
- 支持批量导入和导出 SSH config；导入导出活动栏分为“SSH config 导入导出”“数据库导入导出”和“配置快照”。所有引用私钥的连接都必须由用户逐连接确认绑定，即使当前目录存在同名密钥也不会自动采用
...
- 支持私钥权限检查和一键修复
- 支持 Web 终端，优先使用本地 PTY；macOS 本地 PTY 失败时会继续尝试内置 SSH 远程 PTY，最后才回退普通 SSH 子进程
...
- 支持按天保存系统日志、终端日志和批量执行日志，并支持日志搜索、关键词高亮、命中上下文和自定义保留天数清理
- 支持工作区标签持久化，刷新后恢复转发列表、编辑、导入导出、日志和批量命令等非终端标签
- 桌面端工作区标签支持右键关闭当前、其他、右侧或全部标签
- 启动后自动通过 GitHub Releases 检查最新正式版本；发现更新时除通知外，还会在“设置”活动栏和“关于”入口显示红点，并可在“设置 > 关于”手动检查和打开 Release 页面
- SQLite 保存配置
...
未启用配置加密时，可以继续使用“下载数据库备份”导出 `.db` 文件。
...
启动脚本会自动执行构建。首次 clone 后如果缺少前端组件、TypeScript 或 Electron，脚本会自动执行依赖安装：
